...
page = st.sidebar.radio(
    "Navigate",
    ["Setup", "Outline", "Write", "Memory", "Export", "Aviation Futures"],
    index=0,
)
...
# ===========================================================================
# PAGE: Aviation Futures Intelligence
# ===========================================================================
if page == "Aviation Futures":
    st.title("Aviation Futures Intelligence")
    st.caption(
        "Tracks and synthesises AI in aviation, eVTOL, drone logistics, airport electrification, "
        "aerospace manufacturing, military spillover technologies, and regulatory signals."
    )
    ...
    with col_left:
        st.markdown("**Select domains to include**")
        selected_domains = []
        cols = st.columns(2)
        for i, domain in enumerate(DOMAINS):
            with cols[i % 2]:
                if st.checkbox(domain, value=True, key=f"domain_{i}"):
    ...
    tab_forecast, tab_domain, tab_scenarios = st.tabs(["Full Forecast", "Domain Brief", "Scenario Analysis"])

    # ---------------------------------------------------------------------------
    # Tab: Full Forecast
    # ---------------------------------------------------------------------------
    with tab_forecast:
        st.subheader("Multi-Domain Forecast")
        ...
        if st.button("Run Forecast", type="primary", disabled=not selected_domains):
            with st.spinner("Analysing aviation signals…"):
                ...
                pm = st.session_state.get("project_memory")
                ...
                st.session_state["_avi_forecast"] = result

        result = st.session_state.get("_avi_forecast")
        if result:
    ...
    # ---------------------------------------------------------------------------
    with tab_domain:
        st.subheader("Single-Domain Brief")

        chosen_domain = st.selectbox("Domain", options=DOMAINS, key="domain_brief_select")

        if st.button("Run Domain Brief", type="primary"):
            with st.spinner(f"Analysing {chosen_domain}…"):
                agent = AviationFuturesAgent()
                pm = st.session_state.get("project_memory")
                brief = agent.run_domain_brief(
                    domain=chosen_domain,
                    horizon=horizon,
                    project_memory=pm,
                )
                st.session_state["_avi_domain_brief"] = brief

        brief = st.session_state.get("_avi_domain_brief")
        if brief:
            col1, col2, col3 = st.columns(3)
            ...
            devs = brief.get("key_developments", [])
            if devs:
                st.markdown("### Key Developments")
                for d in devs:
                    st.markdown(f"- {d}")

            gaps = brief.get("infrastructure_gaps", [])
            if gaps:
                st.markdown("### Infrastructure Gaps")
                for g in gaps:
                    st.markdown(f"- {g}")

            signals = brief.get("watch_signals", [])
            if signals:
                st.markdown("### Watch Signals")
                for s in signals:
                    st.markdown(f"→ {s}")
    ...
    with tab_scenarios:
        ...
        st.caption("Explores discrete futures triggered by specific observable events.")

        if st.button("Generate Scenarios", type="primary", disabled=not selected_domains):
            with st.spinner("Building scenario analyses…"):
                agent = AviationFuturesAgent()
                pm = st.session_state.get("project_memory")
                scenarios = agent.run_scenario_analysis(
                    domains=selected_domains,
                    project_memory=pm,
                )
                st.session_state["_avi_scenarios"] = scenarios

        scenarios_result = st.session_state.get("_avi_scenarios")
        if scenarios_result:
